...
import { przetworzone } from "../actions"
...
export function Przetworzone(props: any) {
  ...
  useEffect(() => {
    setTransition(async () => {
      const resp = await przetworzone(query, pagina.limit, pagina.offset)
      ...
      {!panding && <ScrollArea className="h-screen w-auto border">
        ...
        {docs.map((d: any) =>
          <div key={d.guid} className="h-[110px] w-full border-b bg-neutral-100 hover:bg-accent px-2 grid grid-cols-[50px_1fr_1fr_1fr_1fr] py-1">
            ...
            <div className="grid grid-flow-row">
              <InfoTag name="Nr ewid.: ">{d.nrewid}</InfoTag>
              <InfoTag name="Zatwierdził(a): ">{d?.uid_zatwierdzenia}</InfoTag>
              <InfoTag name="Data zatwierdznia: ">{d?.data_zatwierdzenia}</InfoTag>
              <div className="text-slate-900 w-2"><pre> </pre> </div>


            </div>
            <div className="grid grid-flow-row">
              <InfoTag name="Nr dokumentu: ">{d?.numer_dokumentu}</InfoTag>
              <InfoTag name="Wysłał(a) do kancelarii: ">{d.uid_wyslania}</InfoTag>
              <InfoTag name="Data wysłania: ">{d?.data_wyslania}</InfoTag>
              <div className="text-slate-900 w-2"><pre> </pre> </div>
            </div>
