...
import { dekretacja_id, dekretacje_do_oznaczenia, do_przetworzonych_in, zatwierdz_oznaczenie } from "../actions"
import { DekretacjaNaSprawe } from "./dekretacja"



export function DoOznaczenia({
  accounts = null,
  mails = null,
  defaultLayout = [30, 70],
  ...
  const [zrodlo, setZrodlo] = React.useState('Oznaczenie')
  ...
  async function rowClick(event: any) {
    const d = await dekretacja_id(event.pozycja)
    setDekretacja(d)
    setGuid(event)
    setDisplay('dokumenty')
  }

  function isUzytkownikDecydentTest() {
    return "OK";
  }
  async function zatwierdz(guid:any){
    await zatwierdz_oznaczenie(guid)
      ...
      >
          ...
          <Tabs defaultValue="all">
            ...
            {(typListy === 'dokumenty') && <>
              <div className="flex items-center gap-2 px-0 py-2 border-b">
                ...
                <ActionsToolbar zrodlo={zrodlo} multi={true} isDecydent={isUzytkownikDecydentTest} odrzuc={do_przetworzonych_in} zatwierdz={zatwierdz}/>
                <Separator orientation="vertical" className="mx-1 h-6" />
              </div>

              <TabsContent value="all" className="m-0">
                <DokumentList items={mails} onClick={rowClick} zrodlo={dekretacje_do_oznaczenia} guid={guid} query={query} />
              </TabsContent>
            </>}
        ...
        <ResizablePanel defaultSize={defaultLayout[1]}>
          ...
          <ViewCase warunek="dokumenty">
            {(guid != null) && <DokumentDisplay width={displayWidth} guidProvider={guid} >
              <DekretacjaNaSprawe dekretacja={dekretacja} />
              </DokumentDisplay>}
          </ViewCase>
