...
import { purde_do_dekretacje, purde_do_przetworzone, purde_in, zmien_dokument } from "../actions"
...
export function PurdeIn({
  ...
  async function zatwierdz(guid: any) {
    if (isUzytkownikDecydent()) {
      await purde_do_dekretacje(guid)
    }
  }
          ...
          <Tabs defaultValue="all">
            ...
            {(typListy === 'dokumenty') && <>
              ...
              <TabsContent value="all" className="m-0">
                <DokumentList items={mails} onClick={rowClick} zrodlo={purde_in} guid={guid} query={query} />
              </TabsContent>
            </>}
...
export function ZalacznikiZamiana({ guidProvider = null, update = null }: { guidProvider?: any, update?: any }) {
  ...
  return (<>
    ...
    <div className={cn("")}>
      {zalaczniki.map((zal: any, idx: any) => (zal.toLowerCase().endsWith('.pdf')) &&
        <Button variant="outline" key={idx} onClick={async () => {
          await zmien_dokument(guidProvider.guid, idx)
          update()
        }}>{zal}<Replace /></Button>
